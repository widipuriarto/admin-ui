      ...
      <div className="mt-16">
        ...
        <Formik
          ...
          onSubmit={async (values, { setSubmitting }) => {
            try {
              // Kirim data ke function onSubmit yang ada di file signUp.jsx
        ...
        >
          {({ isSubmitting }) => (
            <Form>
              {/* INPUT NAMA */}
              <div className="mb-6">
                <Field name="name">{({ field }) => <LabeledInput {...field} id="name" type="text" label="Name" placeholder="Widi Puriarto" />}</Field>
                <ErrorMessage name="name" component="p" className="text-red-500 text-xs mt-1" />
              </div>

              {/* INPUT EMAIL */}
              <div className="mb-6">
                <Field name="email">{({ field }) => <LabeledInput {...field} id="email" type="email" label="Email Address" placeholder="hello@example.com" />}</Field>
                <ErrorMessage name="email" component="p" className="text-red-500 text-xs mt-1" />
              </div>

              {/* INPUT PASSWORD */}
              <div className="mb-6">
                <Field name="password">{({ field }) => <LabeledInput {...field} id="password" type="password" label="Password" placeholder="••••••••" />}</Field>
                <ErrorMessage name="password" component="p" className="text-red-500 text-xs mt-1" />
              </div>

              <div className="mb-3 text-sm text-gray-03">
                By continuing, you agree to our <a className="text-primary cursor-pointer">terms of service.</a>
              </div>

              {/* TOMBOL BERUBAH JADI LOADING SAAT DISUBMIT */}
